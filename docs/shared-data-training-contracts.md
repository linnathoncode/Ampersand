...
The request does not accept a schema name, data types, nullability, roles, or positions. Nucleus derives the tenant from authentication and reads uploaded tables from that tenant's separate `<tenant_schema>_data` schema. It inspects types and nullability from PostgreSQL, infers roles from the DTO structure, and preserves feature-array order as position.
...
- A deterministic fingerprint prevents duplicate active or completed training for the same snapshot and configuration while allowing failed attempts to be retried.
